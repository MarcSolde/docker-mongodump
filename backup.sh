...
command="mysqldump --host="$MYSQL_HOST" --port="$MYSQL_PORT" --all-databases -uroot -p"$MYSQL_ROOT_PASSWORD""
# else
...
#     done
# else
eval $command > $FILE.sql $filter_errors
    # --archive="$FILE.gz" $filter_errors
